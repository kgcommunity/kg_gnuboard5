...
// 배너출력
function display_banner($position, $prefix='')
{
    global $g5, $is_member;
    ...
    */
    // 배너 출력
    $sql = " select * from {$g5['banner_table']} where bn_position = '$position' order by bn_order, bn_id desc ";
    $result = sql_query($sql);
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        if($position == '10' && $row['bn_type'] == '1') {

            echo "<span style='color:".$row['bn_textcolor'].";background:".$row['bn_bgcolor']."'>" . $row['bn_alt'] . "</span>";

	    }
        else if($position == '23' && $row['bn_type'] == '1') {

            echo "<span style='color:".$row['bn_textcolor'].";background:".$row['bn_bgcolor']."'>" . $row['bn_alt'] . "</span>";

	    }
        else if($position == '3' && $row['bn_type'] == '1') {

            echo '<ul id="tnb">';
                if ($is_member) {
                    if ($is_admin) {
                        echo '<li><a href="'.G5_ADMIN_URL.'"><b>관리자</b></a></li>';
                    }
                    echo '<li><a href="'.G5_BBS_URL.'/member_confirm.php?url='.G5_BBS_URL.'/register_form.php">정보수정</a></li>';
                    echo '<li><a href="'.G5_BBS_URL.'/logout.php">로그아웃</a></li>';
                } else {
                    echo '<li><a href="'.G5_BBS_URL.'/register.php">회원가입</a></li>';
                    echo '<li><a href="'.G5_BBS_URL.'/login.php"><b>로그인</b></a></li>';
                }
            echo '</ul>';

        } else if($row['bn_type'] == '3') {

            if(!$row['bn_skin'])
                $row['bn_skin'] = 'basic';

            echo latest($row['bn_skin'], $row['bn_alt'], $row['bn_url'],"50");
            ...
            $bimg = G5_DATA_PATH.'/banner/'.$prefix.$row['bn_id'];
            if (file_exists($bimg))
            {
                $banner = '';
                ...
                if( $position == '16') { //모바일로고

                    echo '<div style="width:70%;margin:auto;text-align:center;height:80px;background:url(\''.G5_DATA_URL.'/banner/'.$prefix.$row['bn_id'].'\') no-repeat center;background-size:50%"></div>';

                }
                else if( $position == '17') { //모바일 가운데 컨테츠

                    echo $banner.'<img src="'.G5_DATA_URL.'/banner/'.$prefix.$row['bn_id'].'" width="100%" alt="'.$row['bn_alt'].'"'.$bn_border.'>';

                }
                else if( (int)$position >= 18 && (int)$position <= 22 ) { //모바일배너

                    echo $banner.'<div style="float:left;width:20%;text-align:center;"><div class="aimg" style="background:url(\''.G5_DATA_URL.'/banner/'.$prefix.$row['bn_id'].'\') no-repeat center;background-size:60%"></div><div style="margin-top:3px;letter-spacing:-1px;">'.$row['bn_alt'].'</div></div>';

                } else { // PC

                    echo $banner.'<img src="'.G5_DATA_URL.'/banner/'.$prefix.$row['bn_id'].'" width="'.$size[0].'" alt="'.$row['bn_alt'].'"'.$bn_border.'>';
                }
...
function display_main($prefix='main', $suffix='')
{
    global $g5;

    for ($i=1; $i < 6; $i++) {
        $mimg_str = "";
        $mimg = G5_DATA_PATH."/banner/".$prefix.$i.$suffix;
        if (file_exists($mimg)) {
            ...
            if($suffix == '_m') // 모바일
                $mimg_str = '<li><img src="'.G5_DATA_URL.'/banner/'.$prefix.$i.$suffix.'" style="width:100%;"></li>';
            else // PC
                $mimg_str = '<div class="jqb_slide" title="" style="width:100%;background:url(\''.G5_DATA_URL.'/banner/'.$prefix.$i.$suffix.'\') no-repeat center top"></div>';
        }
        if ($mimg_str) {
            echo $mimg_str;
        }
...
function display_maintext_banner($position, $prefix='')
{
    global $g5;

    if (!$position) $position = '15';
    /*
    // 접속기기
    $sql_device = " and ( bn_device = 'both' or bn_device = 'pc' ) ";
    if(G5_IS_MOBILE)
    ...
    */
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        $bn_css .= ($row['bn_alt']) ? 'jqb_text_right' : 'jqb_text_left';
        $bn_css .= ($row['bn_border']) ? ' sbn_border' : '';
        ...
        if (file_exists($bimg))
        {
            ...
            echo $banner.'<div'.$bn_css.'><img src="'.G5_DATA_URL.'/banner/'.$prefix.$row['bn_id'].'" alt=""></div>';
...
function display_sub($no, $prefix='sub')
{
    global $g5;

    $simg_str = "";
    $simg = G5_DATA_PATH."/banner/".$prefix.$no;
    if (file_exists($simg)) {
        ...
        //$simg_str = '<img src="'.G5_DATA_URL.'/banner/'.$prefix.$no.'" width="'.$size[0].'">';
        $simg_str = '<div style="width:100%;height:170px;background:url(\''.G5_DATA_URL.'/banner/'.$prefix.$no .'\') no-repeat center top"></div>';
    }
    if ($simg_str) {
        echo $simg_str;
    }
